...
struct TraceUniforms {
    ...
    camera: vec4<f32>,
    // (escape radius, singularity guard, horizon radius, reserved)
...
fn trace_initialized(initial: InitialState, initial_invariants: Invariants) -> TraceResult {
    ...
    for (var step_index = 0u; step_index < MAXIMUM_STEPS; step_index += 1u) {
        ...
        if termination != 0u {
            ...
            let event_fraction = localized_event_fraction(
                start_value - event_surface,
                end_value - event_surface,
            ...
            let dense_weights = cubic_dense_weights(signed_step, event_fraction);
            ...
            if termination == TERMINATION_ESCAPE {
                direction = normalize(-localized_rhs.spacetime.yzw);
            }
