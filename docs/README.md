...
本文是项目文档与源码上下文的导航入口，不定义产品、物理、API 或当前能力。它按事实类型指出唯一权威来源，帮助读者只加载完成当前任务所需的最小可信上下文。

## 文档层级

不同文件不是一条笼统的“优先级链”，而是各自拥有明确且有限的事实范围：

- manifest、lockfile、源码与 rustdoc 定义版本、feature、签名和当前可执行结构；
- 规范合同定义实现必须满足的产品、物理、验证、架构和平台语义；
- 实现证据只声明当前仓库已经覆盖和仍未覆盖的范围；
- 渲染设计定义候选算法的准入/fallback 原则，路线图只排列尚未闭合的工作；
- research record 保存推导、实验、历史性能、采用/否决理由与恢复条件，不能单独授权 production。

源码与合同不一致时，应判断是实现缺陷还是合同需要显式变更；不能用证据摘要或历史研究记录静默覆盖二者。
...
| 修改 domain 或连续模型       | [数学物理](physics.md) → [验证合同](validation.md)                               | [`gravlume-domain`](../crates/gravlume-domain/)、[`gravlume-reference`](../crates/gravlume-reference/) 及其 fixtures/tests                                                                                                 |
| 修改 GPU trace 或 WGSL       | [数学物理](physics.md) → [验证合同](validation.md) → [架构合同](architecture.md) | [trace](../crates/gravlume-render/src/trace.rs)、[inspection](../crates/gravlume-render/src/trace/)、[WGSL](../crates/gravlume-render/src/shaders/)、[GPU tests](../crates/gravlume-render/src/gpu_trace_tests.rs)与 [GPU 证据](gpu-renderer.md) |
| 修改 publication/lifecycle   | [架构合同](architecture.md) → [平台合同](platform.md)                            | [renderer](../crates/gravlume-render/src/renderer.rs)、[frame](../crates/gravlume-render/src/renderer/frame.rs)、desktop [app](../crates/gravlume-desktop/src/app.rs) 与 native smoke                                      |
...
| 事实                           | 唯一权威来源                                                                                 |
| ------------------------------ | -------------------------------------------------------------------------------------------- |
| Rust 依赖、edition 与 features | [`Cargo.toml`](../Cargo.toml)、[`Cargo.lock`](../Cargo.lock)                                 |
| Python 基线与直接依赖         | [`pyproject.toml`](research/scripts/pyproject.toml)                                           |
| Python 精确解析               | [`uv.lock`](research/scripts/uv.lock)                                                         |
| public Rust interface          | crate source 与 rustdoc                                                                      |
| WGSL entry point、binding/布局 | shader source、host DTO、ABI/GPU tests                                                       |
...
| 候选算法准入与 fallback 原则   | [渲染设计](rendering.md)                                                                     |
| 未完成工作的依赖与退出条件     | [能力路线](roadmap.md)                                                                       |
| 实验、性能历史与候选取舍       | 对应 [research decision record](research/README.md)                                          |

发生冲突时按事实类型核对源码、manifest、lockfile 或对应规范，再修摘要；不要把另一份摘要复制成新的“真相”。
