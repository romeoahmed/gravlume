...
| [连续字段 corpus 首切片](continuous-field-corpus.md)                      | seam/单点 witness 已采用     | test-only ordered batch 已采用；完整 source-edge seed 仍缺独立 BL/Mino artifact               |
| [High-precision BL/Mino witness](high-precision-bl-mino-witness.md)       | canonical 首切片已采用       | `(640,16)` 独立约束 branch/source/frequency/time；其余 corpus 与 artifact schema 仍开放        |
