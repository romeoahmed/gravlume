...
- `Renderer::request_sample_inspection` 接受一个 validated `ImageSample`，在 host 绑定当前 published generation/extent 与 logical sample，并原子返回 `SampleInspectionTicket`。固定执行方法由 `SampleRetrace::METHOD_ID` 标识为 full Kerr–Schild RK4/WGSL binary32；caller 不选择 solver、accelerator 或尚不存在的 quality policy。Private `SampleInspector` 复用 production uniform、surface policy、blackbody LUT、channel model 与 plan-specific scene-value function。
