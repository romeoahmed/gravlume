...
- production renderer 直接形成 scene-linear radiance；scientific capture 负责整帧最终 radiance。Renderer 已提供固定单槽 sample inspection，桌面点击是首个真实消费者；它把实际 published texel 与 fresh full-KS evidence 分开，并以 ticket/completion 闭合 request、lifecycle cancellation、poll 的 generation 与资源语义。连续字段 corpus、独立 high-precision evidence 与第二质量政策仍未闭合。
...
- 有界、按需的 sample inspection，至少携带 process-local ticket、generation/extent/sample、固定 method identity、实际 published `Rgba16Float` texel、fresh full-KS terminal-specific evidence、在 terminal 可证明时的 exact branch key、source kind/anchor、frequency ratio、coordinate-time duration 以及 event/invariant diagnostics；两类像素证据不得混同，`NumericalFailure` 与未经满足预算 refine 的 `Uncertain` 在类型上不得携带 branch；
...
- inspection 的资源上限、读取范围、generation 一致性、取消和错误语义明确；增加 record sink 不静默改变 solver/step policy，另一次 science-quality trace 必须使用新 ticket 与不同 method identity；默认画面不承担隐藏的全帧诊断存储；
